...
def main():
    ...
    # get model
    model_left = Piano2Posi(args)
    model_right = copy.deepcopy(model_left)
    ...
    if args.continue_train:
        ...
        model_left.load_state_dict(state_dict_left)
        model_right.load_state_dict(state_dict_right)

        start_iter = int(model_path_left.split('-iter=')[1].split('-')[0]) + 1
    else:
        start_iter = 0

    optimizer_left = optim.AdamW(model_left.parameters(), lr=args.lr)
    optimizer_right = optim.AdamW(model_right.parameters(), lr=args.lr)

    print('GPU number: {}'.format(torch.cuda.device_count()))
    model_left.to('cuda')
    model_right.to('cuda')
    if torch.cuda.device_count() == 1:
        model_left = torch.nn.DataParallel(model_left)
        model_right = torch.nn.DataParallel(model_right)
    else:
        model_left = BalancedDataParallel(args.gpu0_bs, model_left, dim=0)
        model_right = BalancedDataParallel(args.gpu0_bs, model_right, dim=0)
    ...
    for iteration in pbar:
        ...
        rec_loss_left, vel_loss_left = model_left(audio, frame_num, gt=left_pose)
        rec_loss_right, vel_loss_right = model_right(audio, frame_num, gt=right_pose)

        loss_left = args.weight_rec * rec_loss_left.mean() + args.weight_vel * vel_loss_left.mean()
        loss_right = args.weight_rec * rec_loss_right.mean() + args.weight_vel * vel_loss_right.mean()
        ...
        if (iteration % args.check_val_every_n_iteration == 0 and iteration != 0)  or iteration == args.iterations - 1:
            ...
            with torch.no_grad():
                for val_idx, batch in tqdm(enumerate(valid_loader)):
                    ...
                    pose_hat_left = model_left(audio, frame_num)
                    pose_hat_right = model_right(audio, frame_num)
                    pose_hat = torch.cat([pose_hat_right, pose_hat_left], 2)
            ...
            if iteration % args.save_every_n_iteration == 0  or iteration == args.iterations - 1:
                if np.mean(losss) < best_score:
                    ...
                    if last_checkpoint_path_left is not None:  ## To save storage
                        os.system(f'rm {last_checkpoint_path_left}')
                        os.system(f'rm {last_checkpoint_path_right}')
                    last_checkpoint_path_left = out_dire + '/piano2posi-left-iter=%d-val_loss=%.16f.ckpt' % (iteration, best_score)
                    last_checkpoint_path_right = out_dire + '/piano2posi-right-iter=%d-val_loss=%.16f.ckpt' % (iteration, best_score)
                    torch.save({'state_dict': model_left.module.state_dict(), 'hyper_parameters': args},
                            last_checkpoint_path_left)
                    torch.save({'state_dict': model_right.module.state_dict(), 'hyper_parameters': args},
