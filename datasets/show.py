...
def render_mesh(glctx, vertices, faces, proj, w2c, resolution, is_right, set_color=[120, 150, 255]):
    ...
    light_color = torch.tensor(set_color, device=gt_normals.device)  / 255.0 
    color = torch.sum(gt_normals * light_direction, dim=1, keepdim=True) * light_color + specular * light_color
...
def render_result(out_path, audio,  pose_right, pose_left, video=True, color=[120, 150, 255]):
    ...
    for i in tqdm(range(pose_right.shape[0])):
        ...
        camera_t = pose_right[i, :3]    # guide(position)
        global_orient = pose_right[i, 3:6]  # wrist
        hand_pose = pose_right[i, 6:51]  # 15, 3
        betas = np.zeros(10, dtype=np.float32)  # shape
        ...
        with torch.no_grad():
            render = render_mesh(glctx, vertices, mano_layer['right'].faces, proj, w2c, RESOLUTION, 1, color)
        if video:
        ...
        with torch.no_grad():
            render = render_mesh(glctx, vertices, mano_layer['right'].faces, proj, w2c, RESOLUTION, 0, color)
