...
class PianoPose(Dataset):
        ...
        for up_video_name in data_list:
            ...
            if len(self.up_list) == 0 or (len(self.up_list) > 0 and up in self.up_list):
                ...
                for json_name in json_names:
                    if up == "676539782" and video_name == "BV1QY411T7Xg" and os.path.splitext(json_name)[0] == "BV1QY411T7Xg_seq_0003":
                        continue
    ...
    def __getitem__(self, item, return_param=False):
        if not self.preload:
            ...
            mano_json = os.path.join(self.mano_dir, up, video_name, seq_name + '.json')     # annotation
            audio_path = os.path.join(self.audio_dir, up, video_name, seq_name + '.mp3')    # audio
            audio_array, sampling_rate = librosa.load(os.path.join(audio_path), sr=16000) 
            with open(mano_json, 'r') as f:
                mano_para = json.load(f)
            fps = mano_para['fps']
